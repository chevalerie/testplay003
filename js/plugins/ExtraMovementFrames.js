...
(function() {
    ...
    ImageManager.isEmfCharacter = function(filename) {
        return !!filename.match(/[\(\[][\d\s\,]+[\)\]]/); // check filename for %() or %[]→「%」を消し、「$c_sotai_01(7,0,1,2,3,4,5,6).png」のようにカンマ区切りにする為に書き換えた（2020/05/15）。
    };
    ...
    Game_CharacterBase.prototype.maemfSetupEmfCharacter = function() {
        ...
        if (ImageManager.isEmfCharacter(charName)) {
            var sign = charName.match(/(?:[\(\[])[\d\s\,]+(?:[\)\]])/); //→「%」を消し、「$c_sotai_01(7,0,1,2,3,4,5,6).png」のようにカンマ区切りにする為に書き換えた（2020/05/15）。
    ...
    // characterBlockX - adjust based on number of frames　　by奏ねこま
