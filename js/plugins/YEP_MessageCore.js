...
Window_NameBox.prototype.windowWidth = function() {
    ...
    dw += this.padding * 1;	//NameBoxの幅。デフォルト値は「2」。
...
Window_NameBox.prototype.windowHeight = function() {
    return this.fittingHeight(0.5);	//ここの数値を変更する事で、NameBoxの高さを変更できる（2020/11/16）。
};
...
Window_NameBox.prototype.standardFontSize = function() {	//ネームウィンドウの文字だけ小さくする為、固定値にした。
    return 20;	//デフォルト値は「$gameSystem.getMessageFontSize()」。
};
...
Window_NameBox.prototype.refresh = function(text, position) {
    ...
    var padding = eval(Yanfly.Param.MSGNameBoxPadding) / 3;	//横位置の調整。デフォルト値は「（パラメータ）/2」。
    this.drawTextEx(this._text, padding,-5, this.contents.width);	//ここの数値部分がおそらく、テキストボックスの位置のｙ値。
